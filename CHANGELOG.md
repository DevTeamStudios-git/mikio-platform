...
- **Branch protection corrected to actually enforce** — `main` is now protected by two rulesets: required status checks with **no bypass for anyone** (not even admins), and PR + 1 approval with an admin-role bypass (solo-maintainer merge path). Uses GitHub's Rulesets so the required-approval bypass is scoped to admins *without* also letting admins skip CI (classic `enforce_admins` is all-or-nothing). ⚠️ **Root-cause note:** the ruleset status-check contexts originally used short names (`js-ts`/`python-ai`) that no check-run or commit-status context ever matched — the actual names are `JS/TS — Lint & Test`/`Python/AI — Lint & Test`, each with an **em dash (U+2014)**. As a result, the "required" status checks were **decorative from PR #1 through PR #8**: a red CI never actually blocked a merge in this project's history. PR #9 is the first merge a failing check could have mechanically blocked. Code has always been verified properly by hand (lint/test/build observed directly), but the automated enforcement layer did not exist until the em-dash contexts were set.
- **`frontend`** — shared component library scaffolded per `ARCHITECTURE.md` §8 / `TECH_STACK.md` §2 as `@mikio-ai/frontend`. **Source-first** package (`exports` → `.ts`/`.tsx` sources directly), so consumers transpile it themselves; ships with full devDeps (`react`/`react-dom`/`@types/*`/`typescript`/`vitest`) rather than peer-only deps so it type-checks and tests **standalone** — verified empirically: `tsc --noEmit` and `vitest run` (1 presentational test via `react-dom/server` `renderToString`) both pass in isolation. First component: `Hero` (title + tagline). Consumed by `apps/web` (landing page now renders `<Hero />`). **Verified**: `next build` succeeds with `transpilePackages: ["@mikio-ai/frontend"]` — confirming the predicted landmine that lint+test alone would *not* have caught a broken workspace→app import, which is exactly why the `js-ts` CI job now also runs a full build.
- CI: `js-ts` job now runs `pnpm run build` after lint/test, so turbo's `^build` graph (which compiles `apps/web` and `backend` and forces `transpilePackages` on raw-source workspace packages) executes on every PR instead of only locally.
